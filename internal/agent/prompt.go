...
func BuildSystemPrompt(ctx ClusterContext, providerName string, sendLogs bool, redactionEnabled bool) string {
	...
	b.WriteString("Databases are often EXTERNAL managed services (e.g. cloud RDS) that do NOT run as pods in ")
	b.WriteString("the cluster. They are reached by exec-ing into a running application pod that holds the ")
	b.WriteString("connection string in its environment. `discover_databases` handles this for you and returns, ")
	b.WriteString("for each logical database, the application pod to use as the exec entry point.\n\n")
	b.WriteString("When the user asks about database data or queries:\n\n")
	b.WriteString("1. **Discover**: Use `discover_databases` for the namespace. It returns each reachable database with the podName to use.\n")
	b.WriteString("2. **Credentials**: Optionally `get_db_credentials` (resolved automatically from the pod's connection string; never expose passwords in chat)\n")
	b.WriteString("3. **Schema**: Use `get_db_schema` to understand table structure before querying\n")
	b.WriteString("4. **Query**: Use `query_database` with safe SELECT statements (pass the primary `table` name so results are labelled in the Data Viewer)\n")
	b.WriteString("5. **Follow Relations**: Use `get_foreign_keys` to find related tables, then query them\n\n")
	b.WriteString("### Rules:\n")
	b.WriteString("- ONLY use the podName values returned by `discover_databases`. NEVER guess pod names or target ")
	b.WriteString("`init-*`, `migration-*`, or Completed/Job pods — those are one-shot setup jobs, not databases, ")
	b.WriteString("and every exec into them will fail with 'no credentials' / 'exec failed'.\n")
	b.WriteString("- If `discover_databases` returns nothing, tell the user no reachable database was found in that ")
	b.WriteString("namespace instead of trying random pods.\n")
